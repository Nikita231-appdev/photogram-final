class UserAuthenticationController < ApplicationController
  ...
  def feed
    ...
    other_user = User.where({ :username => the_id })
    @friend = other_user.at(0)

    photo_user = Photo.where({ :owner_id => @friend.id })
    @photo = photo_user


    following_page = FollowRequest.where({ :sender_id => @friend.id, :status => "accepted" })

    nik= Array.new

    following_page.each do |cool|

      tita = cool.recipient_id 
      
      nik.push(tita)

    end 

    @photo_bomb = Photo.where({ :owner_id => a })
  ...
  def index
    ...
    @list_of_users = users_information.order({ :created_at => :desc })
